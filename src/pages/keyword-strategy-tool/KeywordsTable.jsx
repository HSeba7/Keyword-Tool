...
const KeywordsTable = ({ keywords, onCheckboxChange, title, setKeywords, filter, setFilter, hoveredKeyword, setHoveredKeyword }) => {
    ...
    const filterKeywords = (keywords) => {
        ...
        return duplicated.filter(x => x.label.length >= 1);
    };

    const moveFiltered = (key) => {
        const words = keywords.filter(word => word.label.match(key));
        
        words.forEach(word => {
            onCheckboxChange(word.label);
        })
    }
                    ...
                    {filterKeywords(keywords).map((keyword) => (
                        ...
                        >
                            ...
                            >
                                <TextSpliter  
                                    text={keyword.label} 
                                    setKeyword={moveFiltered}
                                    hoveredKeyword={hoveredKeyword}
                                    setHoveredKeyword={setHoveredKeyword}
                                />   
                            </TableCell>
                            <TableCell align='right' sx={{ width: '10%', p: 3, 'span': {p: 0}}}>
                                <Checkbox 
                                    icon={
                                        <img
                                            src={UnCheckedIcon}
