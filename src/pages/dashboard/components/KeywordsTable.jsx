...
const KeywordsTable = ({ keywords, onCheckboxChange, title, setKeywords }) => {
  ...
  const filterKeywords = (keywords) => {
    ...
    if (filterText) {
      duplicated = duplicated.filter(item => item.label.toLowerCase().indexOf(filterText.toLowerCase()) > -1);
    }
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {`${filterKeywords(keywords).length}`} &nbsp; {title}
                </Typography>
                <IconButton
                  onClick={sortKeywords}
                  style={{
                    transform: `rotate(${sortDirection === 1 ? 0 : 180}deg)`, // Rotate based on direct value
        ...
        <TableBody>
          {filterKeywords(keywords).map((keyword) => (
            ...
            >
              ...
              >
                <TextSpliter title={title} text={keyword.label}></TextSpliter>
              ...
              <TableCell align="right" sx={{ width: "10%" }}>
                <Checkbox
                  ...
                  onClick={() => onCheckboxChange(keyword.label)}
                />
              </TableCell>
            </TableRow>
          ))}
